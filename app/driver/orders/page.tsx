...
function StatCard({ label, current, total, color, icon }: StatCardProps) {
	const percentage = useMemo(() => {
		const currentNum = typeof current === 'string' ? parseFloat(current.replace('$', '')) : current;
...
export default function OrdersPage() {
	...
	const tabValue = useMemo(() => TABS[currentTab]?.value || 'Today', [currentTab]);
			...
			<Container maxWidth={false} sx={{ px: { xs: 1, sm: 2 }, pt: 2 }}>
				<Grid container spacing={1.5}>
					...
					<Grid item xs={6}>
						<StatCard
							label="Collected"
							current={`$${parseFloat(String(collectedAmount)).toFixed(0)}`}
							total={`$${parseFloat(String(codAmount)).toFixed(0)}`}
							color={success.main}
