...
export default function SearchItem({open, onClose, items, setItems}: IProps) {
    ...
    useEffect(() => {
        if (debouncedKeywords) {
            const newItems = items.filter((item: IItem) => item.name.toLowerCase().includes(debouncedKeywords.toLowerCase()));
    ...
    <Modal open={open} onClose={onClose}>
        <BoxModal>
            ...
            <Box mt={2} display="flex" flexDirection="column" gap={2}>
                {
                    displayItems.length > 0 ? displayItems.map((item: IItem, index: number) => (
                        <Box key={index} display="flex" gap={1} flexDirection="column">
                            <Typography key={item.id}  sx={{ color: item.availability ? 'black' : grey[500] }}>
                            {`${item.name} - ${
                                !item.availability
                                    ? 'Out of stock'
                                    : item.price === 0
                            ...
                            <TextField 
                                ...
                                variant='outlined'
                                value={item.quantity}
                                onChange={(e: any) => handleOnChangeItem(item, Number(e.target.value))}
