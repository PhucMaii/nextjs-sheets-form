...
import { InputType } from '@/app/form/[id]/type';
...
export default function InputChip({
  ...
  const handleDelete = () => {
    const newInputs = position.inputs.filter((input: InputType) => {
      return input.input_id !== newInput.input_id
    })
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-2">
        ...
        <h1>{newInput.input_name}</h1>
      </div>
      <div className="text-right">
        <button onClick={() => setIsOpenEditModal(true)} className="inline-block text-blue-500 hover:bg-blue-100 focus:ring-4 focus:outline-none focus:ring-blue-200 rounded-lg text-sm p-1.5">
          <svg
