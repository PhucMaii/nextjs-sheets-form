  ...
  return (
    <ShadowSection>
      ...
      <Box display='flex' flexDirection="column" my={3}>
        ...
        <Typography variant="subtitle2" color={grey[500]}>Total Orders</Typography>
      </Box>

    <Box display="flex" flexDirection="column" gap={2}>
     {
        orderSummaryData.map(({color, text, value}, index) => (
            <Box key={index} display='flex' flexDirection="row" gap={1} justifyContent={'space-between'} alignItems="center">
                <Box display="flex" flexDirection="row" gap={2} alignItems="center">
                    <Box sx={{backgroundColor: color, width: 15, height: 15, borderRadius: '50%'}}/>
                    <Typography variant="subtitle1" fontWeight={400}>{text}</Typography>
                </Box>
