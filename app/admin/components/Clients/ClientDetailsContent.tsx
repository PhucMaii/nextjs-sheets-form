...
export default function ClientDetailsContent({orders, dateRange}: IProps) {
    const listOfDateString = useMemo(() => {
        ...
        const dateStringList = generateListOfDateString(normalizedStartDate, normalizedEndDate);
    ...
    const totalSpendBasedOnDate = useMemo(() => {
        ...
        const nonVoidOrders = orders.filter((order: any) => order.status !== 'VOID');
  ...
  return (
    <Box m={2} pr={6}>
        <ClientOrderOverview clientOrders={orders} startDate={dateRange[0]} endDate={dateRange[1]} />

        <ShadowSection width="100%" my={2} sx={{height: 365}}>
            <AreaChart timeSeries={listOfDateString} thisMonthData={totalSpendBasedOnDate} />
        </ShadowSection>
