import { AlertColor, Box, Divider, Grid, MenuItem, Modal, Select, TextField, Typography } from '@mui/material';
...
export default function PasteItemsModal({ open, onClose, currentCategoryId, showNotification, categories }: IProps) {
    ...
    const handlePasteItems = async () => {
        ...
        try {
            ...
            const response = await axios.post(`${API_URL.ADMIN}/categories/pasteItem`, {
    ...
    return (
        <Modal open={open} onClose={onClose}>
            <BoxModal maxHeight="80vh" overflow="scroll">
                <ModalHead heading="Paste Items" onClose={onClose} onClick={handlePasteItems} buttonProps={{loading: isLoading}} buttonLabel='PASTE' />
                ...
                <Box display="flex" flexDirection="column" gap={2}>
                    ...
                    {selectedCategoryId === -1 ? (<ErrorComponent errorText='Please select a category' />) : selectedCategory?.items?.map((item, index: number) => (
                        <Grid container key={index} spacing={2}>
                            <Grid item xs={6}>
                                <TextField
                                    ...
                                    value={item.name}
                                    fullWidth
                                    onChange={(e) => onChangeNewItem('name', e.target.value, index)}
                                />
                            </Grid>
                            ...
                            <TextField
                                ...
                                fullWidth
                                type="number"
                                onChange={(e) => onChangeNewItem('price', +e.target.value, index)}
                                />
                            </Grid>
                        </Grid>
                    ))}
