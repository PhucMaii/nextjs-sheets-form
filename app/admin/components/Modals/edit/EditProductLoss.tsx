...
export default function EditProductLoss({
  ...
  useEffect(() => {
    if (updatedProductLoss.quantityLost && updatedProductLoss.inventoryUnit) {
      setUpdatedProductLoss({
        ...updatedProductLoss,
        totalCost: updatedProductLoss.quantityLost * updatedProductLoss.inventoryUnit.unitPrice,
      });
    }
  ...
  const handleDelete = async () => {
    try {
      setIsDeleting(true);
      const response = await axios.delete(`${API_URL.ADMIN}/product-loss?id=${productLoss.id}`);
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      const response = await axios.put(`${API_URL.ADMIN}/product-loss`, {
      ...
      <BoxModal>
        ...
        <Grid container spacing={2}>
          ...
          >
            ...
            <Typography variant="h5">{productLoss?.inventoryItem?.name}</Typography>
          </Grid>
        ...
        >
          <Box display="flex" alignItems="center" gap={1}>
            <Trash2Icon size={16} />
            <Typography variant="body2" fontWeight="semibold">Delete Product Loss</Typography>
          </Box>
        </LoadingButton>
      </BoxModal>
    </Modal>
