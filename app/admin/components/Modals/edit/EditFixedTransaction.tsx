...
export default function EditFixedTransaction({
  ...
  const handleDelete = async () => {
    try {
        setIsDeleting(true);
      const res = await axios.delete(`${API_URL.ADMIN}/fixed-transactions`, {
  ...
  const handleUpdate = async () => {
    try {
      setIsUpdating(true);
      const res = await axios.put(`${API_URL.ADMIN}/fixed-transactions`, {
      ...
      <BoxModal maxHeight="90vh" overflow="auto">
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} display="flex" flexDirection="column" gap={1}>
            ...
            >
              {isDeleting ? (
                  <Typography variant="body1" color={grey[500]} fontWeight="semibold">
                    Deleting...
                  </Typography>
              ) : (
                <Box display="flex" alignItems="center" gap={1}>
                  <Trash2Icon />
                  <Typography variant="body1" color="error" fontWeight="semibold">
                    Delete
                  </Typography>
                </Box>
              )}
