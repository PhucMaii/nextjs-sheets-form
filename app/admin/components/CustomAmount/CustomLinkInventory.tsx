...
const CustomLinkInventory = ({showNotification, onClose, orderId, setItemList, onUpdateUI}: IProps) => {
    ...
    const { selectedUnit, AddUnitModal, EditUnitModal, UnitDisplay } =
        useEditUnit(customAmount.units, customAmount.inventoryUnit, showNotification, false);

    useEffect(() => {
      if (promptedItem.id !== -1) {
        let units = promptedItem?.vendorItem?.flatMap(
            (item: any) => item.unit,
        ) || [];

        units = Array.from(
            new Map(
                units?.map((unit: any) => [unit.ratio, unit]),
            ).values(),
        );
    ...
    const onAddCustomAmountDB = async () => {
        ...
        setIsLoading(true);
        try {
          const response = await axios.post(`${API_URL.ADMIN}/custom-amount/link-inventory`, {
            orderId,
            customAmount,
          });
    ...
    <>
        ...
        {EditUnitModal}
            ...
            <TextField
                ...
                onChange={(e) => setCustomAmount({...customAmount, name: e.target.value})}
            ...
            <TextField
                ...
                onChange={(e) => setCustomAmount({...customAmount, quantity: +e.target.value})}
                placeholder='Enter quantity...'
            />

            <LoadingButton onClick={addCustomAmount} loading={isLoading} variant="contained">
                Add Custom Amount
            </LoadingButton>
        </Box>
    </>
...
export default memo(CustomLinkInventory, (prev, next) => {
  return (
    JSON.stringify(prev.showNotification) === JSON.stringify(next.showNotification)
  );
})