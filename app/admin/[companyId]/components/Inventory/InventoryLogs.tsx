...
export default function InventoryLogs() {
  ...
  const { data: inventoryItems, isLoading: isLoadingInventoryItems } = useQuery(
    {
      ...
      queryFn: async () => {
        const response = await axios.get(
          getAdminApiUrl(companyId, '/inventory?includedInternal=true'),
        );
  ...
  const groupedLogs = useMemo(() => {
    ...
    if (typeFilter !== 'all') {
      displayLogs = displayLogs?.filter((log: any) => log.type === typeFilter);
    }
    return groupLogsByDate(displayLogs || []);
  }, [filteredLogs, typeFilter]);
  ...
  useEffect(() => {
    if (debouncedSearch) {
      const searchedLogs = logs.filter((log: any) => {
        return (
          log.log.toLowerCase().includes(debouncedSearch.toLowerCase()) ||
          log.type.toLowerCase().includes(debouncedSearch.toLowerCase()) ||
          log.createdFrom.toLowerCase().includes(debouncedSearch.toLowerCase())
        );
      });
    ...
    return (
      <Box>
        {sortedDate.map((date) => {
          ...
          const stockIn = logsArray
            .filter(
              (log: any) =>
                log.type === InventoryLogType.STOCK_IN ||
                log.type === InventoryLogType.RESTOCK,
            )
            .reduce((sum: number, log: any) => sum + log.quantity, 0);
          const stockOut = logsArray
            .filter(
              (log: any) =>
                log.type === InventoryLogType.SUBTRACT ||
                log.type === InventoryLogType.LOST,
