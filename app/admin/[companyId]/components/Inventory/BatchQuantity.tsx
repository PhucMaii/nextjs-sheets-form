  ...
  const handleDeleteFifo = async (fifo: IFifo) => {
    ...
    if (fifo?.orderedItems?.length > 0) {
      if (fifoIndex + 1 >= fifoList.length) {
        showNotification(
          'error',
          'Cannot delete the last fifo has item affected',
        );
        return;
      }
    ...
    try {
      const response = await axios.delete(
        `${getAdminApiUrl(companyId, `/inventory/fifo?id=${fifo.id}&nextFifoId=${nextFifoId}`)}`,
      );
  ...
  const handleUpdateQuantity = async () => {
    try {
      ...
      const response = await axios.put(
        getAdminApiUrl(companyId, '/inventory/fifo'),
        {
          fifoId: fifo.id,
          updatedQuantity,
        },
