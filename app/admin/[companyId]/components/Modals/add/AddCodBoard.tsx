...
import { COD_STATUS, getAdminApiUrl } from '@/app/utils/enum';
...
export default function AddCodBoard({
  ...
  const [drivers] = SWRFetchData(
    getAdminApiUrl(companyId, `/drivers?date=${date}`),
  );
  ...
  const addNewBoard = async () => {
    setIsLoading(true);
    try {
      const response = await axios.post(getAdminApiUrl(companyId, '/cod'), {
        date,
        note: newBoard.note,
        cash: newBoard.cash,
