...
import { WORKING_ROLE, getAdminApiUrl } from '@/app/utils/enum';
...
export default function EditShift({
  ...
  const fetchDrivers = async () => {
    try {
      const response = await axios.get(getAdminApiUrl(companyId, '/drivers'));
  ...
  const handleUpdateShift = async () => {
    setIsUpdating(true);
    try {
      const response = await axios.put(getAdminApiUrl(companyId, '/shifts'), {
        ...updatedShift,
  ...
  const handleDeleteShift = async () => {
    ...
    try {
      const response = await axios.delete(
        getAdminApiUrl(companyId, `/shifts?id=${updatedShift.id}`),
      );
