...
const useOrdersData = (companyId: string, date: string, currentStatus: ORDER_STATUS | PaymentStatus) => {
...
const useMenuState = () => {
	const [actionButtonAnchor, setActionButtonAnchor] = useState<null | HTMLElement>(null);
...
const filterOrderByRoute = (orders: Order[], routes: any, currentRoute: number, baseOrderData: Order[], setRouteOrders: (orders: Order[]) => void) => {
...
export default function Orders() {
	...
	const queryKeywords = searchParams?.get('q');
	...
	const virtuosoHeight = useMemo(() => {
		if (totalPosition.current) {
			...
			return currentOffSetHeight - totalPosition.current?.getBoundingClientRect().bottom;
		}
	...
	}, [debouncedKeywords, ordersState.baseOrderData, ordersState.currentRoute, queryKeywords]);

	useEffect(() => {
		generateOrderData();
	}, [ordersState.currentRoute, ordersState.currentPage, ordersState.filterOptions]);

	useEffect(() => {
		if (ordersState.incomingOrder) {
			if (
				ordersState.incomingOrder.deliveryDate === date &&
				(ordersState.incomingOrder.status === ordersState.currentStatus || ordersState.tabIndex === 0) &&
				!ordersState.incomingOrder.isReplacement
			) {
				ordersState.setBaseOrderData((prevOrders) => [ordersState.incomingOrder!, ...prevOrders]);
			} else if (ordersState.incomingOrder.isReplacement || ordersState.incomingOrder.isVoid) {
				const newOrderData = ordersState.baseOrderData.filter((order: Order) => {
					return order.id !== ordersState.incomingOrder!.id;
				});
				ordersState.setBaseOrderData([ordersState.incomingOrder!, ...newOrderData]);
			}
	...
	const generateOrderData = useCallback((orderList = ordersState.baseOrderData) => {
	...
	}, [ordersState.baseOrderData, ordersState.currentRoute, ordersState.filterOptions, ordersState.currentPage, routes]);
	...
	const handleUpdateStatus = useCallback(async (status: ORDER_STATUS | PaymentStatus, type: 'fulfill' | 'payment' = 'fulfill'): Promise<void> => {
		ordersState.setIsExecutingAction(true);
		try {
			await updateStatus(companyId, status, ordersState.selectedOrders, showNotification, type);
			mutate();
	...
	const handleSelectOrder = useCallback((e: any, targetOrder: Order) => {
		e.stopPropagation();
		e.preventDefault();
		...
		if (selectedOrder) {
			const newSelectedOrders = ordersState.selectedOrders.filter((order: Order) => {
				return order.id !== targetOrder.id;
			});
			ordersState.setSelectedOrders(newSelectedOrders);
		} else {
			ordersState.setSelectedOrders([...ordersState.selectedOrders, targetOrder]);
		}
	...
	const renderUpperContent = () => (
		<>
			...
			<OrderOverview
				...
				currentDate={date}
				orderData={ordersState.currentRoute === 0 ? ordersState.baseOrderData : ordersState.routeOrders}
			...
			<Grid container alignItems="center" spacing={1}>
				<Grid item xs={12} md={10.5}>
					<Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
						<Tabs
							...
							value={ordersState.tabIndex}
							onChange={(e, newValue) => ordersState.setTabIndex(newValue)}
				...
				<Grid item xs={6}>
					<FormControlLabel
						control={
							<Checkbox
								checked={ordersState.orderData.length === ordersState.selectedOrders.length}
				...
				<Grid item xs={6} textAlign="right">
					<Box display="flex" gap={1}>
						...
						<Box gap={2}>
							<IconButton onClick={(e) => menuState.setFilterAnchor(e.currentTarget)}>
								<TuneIcon />
							</IconButton>
							...
							>
								<MenuItem
									style={{
										backgroundColor:
											ordersState.filterOptions.length === 0 ? grey[200] : '#ffffff',
									}}
								...
								<MenuItem
									...
									onClick={() => ordersState.setFilterOptions([PAYMENT_TYPE.COD])}
								>
								...
								<MenuItem
									style={{
										backgroundColor:
											ordersState.filterOptions.length === 1 && ordersState.filterOptions[0] === wcodDay
												? grey[200]
												: '#ffffff',
									}}
								...
								<MenuItem
									style={{
										backgroundColor:
											ordersState.filterOptions.length === 2 ? grey[200] : '#ffffff',
									}}
									onClick={() => ordersState.setFilterOptions([PAYMENT_TYPE.COD, wcodDay])}
								>
					...
					<>
							...
							itemContent={(index, order) => {
								return (
									<OrderAccordion
										...
										handleOpenDetails={() => ordersState.setSelectedOrderDetails(order)}
										mutateOrders={mutate}
									/>
								);
							}}
						...
						<div style={{ width: '100%' }}>
							<Pagination
								...
								shape="rounded"
								size="large"
								onChange={(e: any, value: number) => ordersState.setCurrentPage(value)}
							/>
						</div>
					</>
				) : (
	...
	return (
		<Sidebar>
			...
			<div style={{ display: 'none' }}>
				<MemoizedAllPrint
					orders={ordersState.selectedOrders.length > 0 ? ordersState.selectedOrders : ordersState.orderData}
					ref={componentRef}
				/>
			</div>
			<DeleteModal
