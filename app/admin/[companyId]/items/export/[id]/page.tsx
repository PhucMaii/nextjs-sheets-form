...
import ExportCategory from '@/app/admin/[companyId]/components/Printing/ExportCategory';
import LoadingComponent from '@/app/components/LoadingComponent/LoadingComponent';
import { ShadowSection } from '@/app/admin/[companyId]/reports/styled';
import EditItemImage from '@/app/admin/[companyId]/components/Modals/EditItemImage';
...
const ItemExportPage = () => {
  ...
  const fetchBaseItems = async () => {
    const items = await fetchApi(
      getAdminApiUrl(companyId, `/items?categoryId=${id}`),
    );
    setBaseItems(items);
    setIsLoading(false);
  };
  ...
  const columns: GridColDef[] = [
    {
      ...
      renderCell: (params: any) => {
        if (!params.row?.image && !params.row?.inventoryItem?.image)
          return null;
        return (
          // <img
