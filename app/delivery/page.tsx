...
export default function CashFlow() {
  ...
  const handleAddSheet = (e: MouseEvent) => {
    e.preventDefault();
    if (sheetName === '') {
      return;
    }
    setForm((prevForm: SheetForm[]) => {
      return [
        ...prevForm,
        {
          sheetName,
  ...
  const handleRemoveSheet = (e: MouseEvent, id: number) => {
    e.preventDefault();
              ...
              <div>
                {form.map((sheet, index) => {
                  return (
                    <div
                      key={index}
                      className="relative flex justify-between select-none items-center whitespace-nowrap rounded-lg bg-gray-900/10 py-1.5 px-3 mt-2 font-sans text-xs font-bold uppercase text-gray-900"
                    >
                      <span className="">
                        Sheet Name: {sheet.sheetName}, Row: {sheet.row}
                      </span>
                      <button
                        onClick={(e: MouseEvent) => handleRemoveSheet(e, index)}
