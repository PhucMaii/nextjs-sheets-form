...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
    try {
        ...
        const nonVoidOrders = orderList.filter((order: Order) => order.status !== ORDER_STATUS.VOID);
        ...
        const clientRoutes = nonVoidOrders.map((order: Order) => {
            ...
            const relatedRoute = order.user?.routes?.find((route: any) => routeMap.has(route.routeId));
        ...
        for (const route of dayRoutes) {
            ...
            const sortedUserIdsMap: any = new Map(sortedUserIds.map((id: any, index: number) => [id, index]));
            // console.log(sortedUserIdsMap, 'sortedUserIdsMap');

            const currentRouteOrders = orderByRoutes.filter((order: Order) => order.routeId === route.id);

            currentRouteOrders.sort((orderA: Order, orderB: Order) => {
                return sortedUserIdsMap.get(orderA.userId) - sortedUserIdsMap.get(orderB.userId);
            });
            // console.log(orderByRoutes.length, 'orderByRoutes.length');
