...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
    try {
        ...
        if (id) {
            ...
            if (status === TRANSACTION_STATUS.PAID && existingExpense.paymentMethodId === otherPaymentMethodId && newPaymentMethodId === otherPaymentMethodId) {
            ...
            if (existingExpense.paymentMethodId === otherPaymentMethodId && newPaymentMethodId !== otherPaymentMethodId && status === TRANSACTION_STATUS.PAID) {
        ...
        if (idsToUpdate) {
            ...
            // Handle update other payment method status
            if (newPaymentMethodId !== otherPaymentMethodId && status === TRANSACTION_STATUS.PAID) {
                await prisma.expense.updateMany({
                    where: {
                        id: {
                            in: idsToUpdate
