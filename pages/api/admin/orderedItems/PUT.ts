...
const generateScheduleOrderItems = (
  ...
  const newItems = updatedItems.map((newItem: UpdatedItem) => {
    const { id, orderId, totalPrice, ...restOfData } = newItem;
    ...
    return { ...restOfData, scheduledOrderId: scheduleOrder.id };
  });
...
const updateScheduleOrderItems = async (
  ...
  const newTotalPrice: number = newItems.reduce((acc: number, newItem: OrderedItems) => {
