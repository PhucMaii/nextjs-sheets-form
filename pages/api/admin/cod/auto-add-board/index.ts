...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    ...
    if (boards.length > 0 && boardOrders.length > 0) {
      await insertOrdersToSelectedBoards(boardOrders, boards, routeOnDate, todayString);
      return res.status(200).json({
    ...
    await insertOrdersToSelectedBoards(dateOrders, newBoards, routeOnDate, wcodDay);
