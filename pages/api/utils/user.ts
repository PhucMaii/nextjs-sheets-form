...
export const categorizeUser = async (userId: number) => {
    ...
    try {
        ...
        const firstDayOfLast3Months = new Date(now.getFullYear(), now.getMonth() - 3, 1);
        const listOfDateString = generateListOfDateString(firstDayOfLast3Months, lastDayOfLastMonth);
        ...
        const ordersRevenue = userOrdersLast3Months.reduce((acc: number, order: Orders) => {
